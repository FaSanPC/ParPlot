...
var width = document.body.clientWidth,
    height = d3.max([document.body.clientHeight-270, 240]);
...
var colors = [];
d3.json("csv/pc_clr.json", function(json) {
  colors = json;
//var colors = {
      //"Alternative 0"                  : [177,100,50], //#77B9B9
      //"Unregulated regime"             : [310,85,50], //'#E23DC9',
      //"exp5.1.4_1M"                    : [210,73,32], //
      //"SDP"                            : [359,69,49]  //
      //};
});
...
interesting = document.getElementById('interesting').getContext('2d');
//interesting.strokeStyle = "rgba(255,140,0,1)";
//Add a placeholder function for browsers that don't have mozDash()
if (!interesting.mozDash) {
    interesting.mozDash = function () {}
    interesting.lineWidth = 8;
};
    interesting.mozDash = [15,7];
    interesting.lineWidth = 2;
...
// Load the data and visualization
  d3.csv("csv/data_test.csv", function(raw_data) {
  ...
  //data
  //MM: add reference data: 
  d3.csv("csv/data_ref_test.csv", function(raw_data_) {

  // Convert quantitative scales to floats
  data_r = raw_data_.map(function(dr) {
    for (var k in dr) {
      if (!_.isNaN(raw_data_[0][k] - 0) && k != 'id') {
        dr[k] = parseFloat(dr[k]) || 0;
      }
    };
    return dr;
  });
  ...
  brush();
  //path(data[0],foreground, colors[0]);
  });
});
...
// On click show name of the selected solution in another division
function setInGraph(d){
//  d3.select("#chart").append('text').text('la puta que te pario')   
...
function dataName(d){
    ...
    var names = ['Name:'+d.name];

	var ul = d3.select('#extra-info').append('text');
	...
	.append('li')
	.html(String)
    .attr("id", "paragraphInfo");
  d3.select("#extra-info")
    //.text("Name:"+d.name+"-Group:"+d.group+"-First Axis:"+d.fl_dmgHNo+"-Second Axis:"+d.X_fl_dmgHNo+"-Third Axis:"+d.hp_hpdef_w_p_RR+"-Fourth Axis:"+d.ws_meandef2+"-Fifth Axis:"+d.X_ws_meandef2)
    
//Just in case of More info button    
  //.append("text")
    //.text("More info")
   ...
   //.on("click", function() { //window.open("http://www.polimi.it/"); });
}
...
function brush() {
  ...
  if (selected.length < data.length && selected.length > 0) {
    ...
    d3.select("#clear-search").attr("disabled", null);
  } else {
  ...
  // Render references, outside filter
  for (var i=0;i<data_r.length;i++) 
  {
  if (data_r[i]) {path(data_r[i],interesting,color(data_r[i].group,0.85))};
  };
}
...
d3.select("#clear-search").on("click", refresh);
d3.select("#open-file").on("click", openFile);
...
function openFile(){
   document.getElementById('my_file');
}
